...
export default function OwnerLayout({ children }: { children: ReactNode }) {
  ...
  const navItems = [
    ...
    { name: "My Apartments", href: "/owner/apartments", icon: Grid },
      ...
      <header className="hidden md:flex items-center justify-between px-8 py-4 border-b border-border bg-card">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-lg">Owner Portal</span>
          </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.name} href={item.href}>
                  <div className={`text-sm font-medium transition-colors cursor-pointer ${isActive ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`}>
                    {item.name}
                  </div>
                </Link>
              );
